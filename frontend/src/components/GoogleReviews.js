...
import { UnifiedSection, UnifiedContainer, UnifiedHeading } from "./ui/UnifiedSection";
    ...
    <UnifiedSection variant="compact">
      <UnifiedContainer>
        <div className="text-center mb-12">
          <UnifiedHeading level={2}>Das sagen unsere Kunden</UnifiedHeading>
          <div className="flex items-center justify-center space-x-4 mb-6">
            <div className="flex items-center space-x-1">{[1,2,3,4,5].map((star) => (<Star key={star} className="w-5 h-5 text-acencia-accent fill-acencia-accent" />))}</div>
          ...
          <div className="relative -mx-6">
            <div className="flex space-x-6 animate-scroll-smooth px-6">
              {tripleReviews.map((review, index) => (
                <div key={`${review.id}-${index}`} className="flex-shrink-0 w-80 group">
                  <div className="bg-black/30 backdrop-blur-sm rounded-xl p-6 border border-white/10 hover:bg-black/40 hover:scale-[1.02] transition-all duration-300 h-full">
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex items-center space-x-3">
                        <div className="w-10 h-10 bg-gradient-to-br from-gray-600 to-gray-700 rounded-lg flex items-center justify-center text-white font-semibold text-sm">{review.initials}</div>
                        <div><h4 className="font-semibold text-white text-base">{review.name}</h4><p className="text-gray-300 text-xs">{review.position}</p><p className="text-gray-400 text-xs">{review.date}</p></div>
                      </div>
                    </div>
                    <div className="flex items-center space-x-1 mb-4">{[1,2,3,4,5].map((star) => (<Star key={star} className="w-4 h-4 text-acencia-accent fill-acencia-accent" />))}</div>
        ...
        <div className="text-center mt-12">
          <div className="inline-flex items-center space-x-6 bg-black/20 rounded-xl px-6 py-3">
            <div className="flex items-center space-x-2"><div className="w-2 h-2 bg-green-500 rounded-full"></div><span className="text-gray-300 text-sm font-medium">Verifizierte Bewertungen</span></div>
            <div className="h-3 w-px bg-white/20"></div>
            <div className="flex items-center space-x-2"><span className="text-acencia-accent font-bold text-base">100+</span><span className="text-gray-300 text-sm">Zufriedene Kunden</span></div>
          </div>
        </div>
      </UnifiedContainer>
    </UnifiedSection>
