    ...
    <UnifiedSection variant="standard">
      <UnifiedContainer>
        <div className="text-center mb-16">
          <UnifiedHeading level={2}>Für jede Unternehmensgröße</UnifiedHeading>
          <p className="text-lg text-gray-300 max-w-2xl mx-auto leading-relaxed font-normal">Maßgeschneiderte bAV-Lösungen, die mit Ihrem Unternehmen wachsen</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-7xl mx-auto">
          {segments.map((segment) => (
            <div key={segment.id} className="flex flex-col items-center">
              <div className="relative w-full h-36 bg-black/30 backdrop-blur-sm rounded-3xl border border-white/10 hover:bg-black/40 cursor-pointer transition-all duration-500 ease-out group hover:-translate-y-2 hover:scale-105 flex items-center justify-center pt-7 mb-5" style={{ borderRadius: '22px' }}>
                <div className="absolute -top-9 left-1/2 transform -translate-x-1/2 w-24 h-24 grid place-items-center group-hover:-translate-y-1 transition-transform duration-500" style={{filter: 'drop-shadow(0 7px 14px rgba(0,0,0,.2))'}}>
                  <svg className="w-24 h-24 absolute inset-0" viewBox="0 0 100 100">
                    <polygon points="50,5 90,27.5 90,72.5 50,95 10,72.5 10,27.5" fill="#092130" stroke="#ecd2b8" strokeWidth="2.5" opacity="0.98" />
                    <polygon points="50,14 82,32 82,68 50,86 18,68 18,32" stroke="#ecd2b8" strokeWidth="5" fill="none" className="group-hover:stroke-[7] transition-all duration-300" />
                  </svg>
                  <div className="absolute inset-0 flex items-center justify-center"><div className="w-12 h-12 flex items-center justify-center">{segment.icon}</div></div>
                </div>
                <div className="w-11/12 h-11 rounded-xl flex items-center justify-center font-semibold text-sm tracking-wide relative overflow-hidden px-3" style={{ background: 'linear-gradient(135deg, #ecd2b8 0%, #e8c9a8 100%)', color: '#0b2a3d', fontSize: '15px', fontWeight: '600', letterSpacing: '.3px', boxShadow: 'inset 0 2px 4px rgba(255,255,255,.4), inset 0 -2px 4px rgba(0,0,0,.05), 0 2px 8px rgba(0,0,0,.08)' }}>
                  <span className="relative z-10 text-center leading-tight">{segment.title}</span>
                  <div className="absolute top-0 w-full h-full transition-all duration-500 group-hover:left-full" style={{ left: '-100%', background: 'linear-gradient(90deg, transparent, rgba(255,255,255,.3), transparent)' }}></div>
                </div>
                <div className="absolute left-1/2 transform -translate-x-1/2 rounded-full" style={{ bottom: '12px', width: '55%', height: '2px', background: 'linear-gradient(90deg, transparent, rgba(236,210,184,.5) 20%, rgba(236,210,184,.7) 50%, rgba(236,210,184,.5) 80%, transparent)', opacity: '0.8' }}>
                  <div className="absolute left-0 right-0 rounded-full" style={{ top: '-2px', height: '6px', background: 'inherit', filter: 'blur(4px)', opacity: '0.6' }}></div>
                </div>
              </div>
        ...
        <div className="text-center mt-16">
          <UnifiedButton onClick={() => window.open(APP_CONFIG.BOOKING_URL, '_blank')}>Beratung für Ihr Unternehmen</UnifiedButton>
        </div>
      </UnifiedContainer>
    </UnifiedSection>
  );
